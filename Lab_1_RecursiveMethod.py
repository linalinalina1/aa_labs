...
# Choose input set
RUN_MODE = "small"   # "small" or "big" (big will be very slow)
...
def benchmark_3runs(func, inputs):
    ...
    for run_idx in range(RUNS):
        row = []
        for n in inputs:
            print(f"{RUN_MODE.upper()} | Recursive | run {run_idx+1}/{RUNS} | computing for n = {n} ...")
...
def plot_average_graph(inputs, avg_times):
    ...
    plt.title("Recursive Fibonacci Function (Average of 3 runs)")
    plt.xlabel("n-th Fibonacci Term")
    plt.ylabel("Time (s)")
    plt.grid(True)

    if SAVE_GRAPH:
        filename = f"recursive_avg_{RUN_MODE}.png"
...
def main():
    if RUN_MODE == "small":
        inputs = SMALL_INPUTS
        title = "Results for Recursive Fibonacci Method (Small Inputs) — 3 runs + AVG"
    elif RUN_MODE == "big":
        inputs = BIG_INPUTS
        title = "Results for Recursive Fibonacci Method (Big Inputs) — 3 runs + AVG"
    else:
        raise ValueError("RUN_MODE must be 'small' or 'big'")

    print(f"Running Recursive Fibonacci benchmark with RUN_MODE='{RUN_MODE}', RUNS={RUNS}")
    runs_times, avg_times = benchmark_3runs(fib_recursive, inputs)
